...
add_thickbox();
$wp_genericrecommended_plugins = array(
	...
	<div class="feature-section recommended-plugins">
		<?php foreach ( $wp_genericrecommended_plugins as $plugin => $prop ) { ?>
		<?php
		...
		switch ( $active['needs'] ) {
			...
			$label = __( 'Install', 'wp-generic' );
			break;
			case 'activate':
			$class = 'activate-now button button-primary';
			$label = __( 'Activate', 'wp-generic' );
			break;
			case 'deactivate':
			$class = 'deactivate-now button';
			$label = __( 'Deactivate', 'wp-generic' );
			break;
		}
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ): ?>
				<span class="recommended"><?php _e( 'Required', 'wp-generic' ); ?></span>
			<?php endif; ?>
			...
			<div class="b-wrap">
			<span class="version"><?php echo __( 'Version:', 'wp-generic' ); ?><?php echo esc_html( $info->version ) ?></span>
