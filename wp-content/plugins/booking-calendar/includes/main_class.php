...
class wpdevart_Main {
	...
	public function __construct($id = 0, $widget = false){
		...
		$calendar_title = $this->calendar_model->get_calendar_rows($this->id);
		$this->calendar_title = $calendar_title["title"];
	...
	**/
	...
	public function main_booking_calendar($id, $res_id = 0, $date='', $ajax = false, $selected = array(),$data = array(),$submit = "",$hours = array()){
		...
		foreach($text_for as $key => $for) {
			if(isset($this->theme_option['use_mo']) && $this->theme_option['use_mo'] == "on") {
				...
				$for_trarray[$key] = $this->theme_option[$key];
			} else {
				$for_trarray[$key] = __($for,'booking-calendar');
			}
		}		
		...
		$calendar_start = new wpdevart_bc_BookingCalendar($date, $res, $id, $this->theme_option, $this->calendar_data, $this->form_option, $this->extra_field,array(),false,$this->widget,$for_trarray,$this->calendar_title);
		...
		if(isset($data["wpdevart-submit".$submit])){
			...
			if(!is_admin() || count($data)) {
				...
				if(isset($this->theme_option["action_after_submit"]) && $this->theme_option["action_after_submit"] == "stay_on_calendar") {
					$submit_message = $this->theme_option["message_text"];
				}
				else { 
		...
		$calendar = new wpdevart_bc_BookingCalendar($date, $res, $id, $this->theme_option, $calendar_data_after_save, $this->form_option,$this->extra_field, $selected,$ajax,$this->widget,$for_trarray,$this->calendar_title);
		...
		<script>
			var wpdevartBooking<?php echo $this->booking_id; ?> = {
				...
				id : <?php echo $id; ?>,
				min : <?php echo ((isset($this->theme_option["min_days"]) && $this->theme_option["min_days"] != '') ? $this->theme_option["min_days"] : 1); ?>,
				max : <?php echo ((isset($this->theme_option["max_days"]) && $this->theme_option["max_days"] != '') ? $this->theme_option["max_days"] : 1000); ?>,
				min_hour : <?php echo ((isset($this->theme_option["min_hours"]) && $this->theme_option["min_hours"] != '') ? $this->theme_option["min_hours"] : 1); ?>,
				max_hour : <?php echo ((isset($this->theme_option["max_hours"]) && $this->theme_option["max_hours"] != '') ? $this->theme_option["max_hours"] : 1000); ?>,
		...
		if (!$ajax) {
			if(isset($this->theme_option["type_days_selection"]) && $this->theme_option["type_days_selection"] == "multiple_days") {
				$booking_calendar .= "<div class='error_text_container error_days div-for-clear'><span class='error_text'>".$for_trarray["for_error_multi"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
				$booking_calendar .= "<div class='error_text_container error_night div-for-clear'><span class='error_text'>".$for_trarray["for_night"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
				$booking_calendar .= "<div class='error_text_container error_min_day div-for-clear'><span class='error_text'>".$for_trarray["for_min"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
				$booking_calendar .= "<div class='error_text_container error_max_day div-for-clear'><span class='error_text'>".$for_trarray["for_max"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
				$booking_calendar .= "<div class='error_text_container error_min_hour div-for-clear'><span class='error_text'>".$for_trarray["for_min_hour"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
				$booking_calendar .= "<div class='error_text_container error_max_hour div-for-clear'><span class='error_text'>".$for_trarray["for_max_hour"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
			} else {
				$booking_calendar .= "<div class='error_text_container div-for-clear'><span class='error_text'>".$for_trarray["for_error_single"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
			}
		}	
		$booking_calendar .= "<div class='booking_calendar_main'>";
		if( isset($data["wpdevart-submit".$submit]) && count($payments)) {
			if(isset($result[2]))
				$result = $result[2];
			if( isset($this->theme_option["show_booking_info"]) && $this->theme_option["show_booking_info"] == "on") {
				$booking_calendar .= $this->show_reservation_info($result);
			}
			$booking_calendar .= '<div class="wpdevart_order"><form method="post" id="wpdevart_order_'.$submit.'">';
			...
			foreach($payments as $key=>$payment){
				...
				$booking_calendar .= '<button type="button" class="wpdevart-payment-button ' . $button_cl . ' ' . $class . '" id="'.$key.'" name="wpdevart-payment'.$submit.'" data-id="'.$submit.'" data-resid="'.(isset($result)? $result['reservation_id'] : "").'" data-themeid="'.$this->ids["theme_id"].'">'.$button_content .'</button>';
			}
			...
			$booking_calendar .= '<input type="hidden" name="resid_'.$submit.'" value="'.(isset($result)? $result['reservation_id'] : "").'">
	...
	private function show_reservation_info($data) {
		...
		$countries = $this->reservation_model->get_countries();
		...
		if(count($data)) {
			...
			if(isset($data["sale_percent"]) && !empty($data["sale_percent"])){
				$sale_percent = ($data["total_price"]  * 100) / (100 - $data["sale_percent"]);
				$sale_percent_html = (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data["currency"] : '') . $sale_percent . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data["currency"] : '')) . " - " . $data["sale_percent"] . "% = ";
			}
			if($hour_html != "") {
				$date .= "<div>".__("Hour","booking-calendar")." ".$hour_html."</div>";
			}
			$reserv_info .= self::reservation_item(__('Reservation ID','booking-calendar'),$data["reservation_id"]);
			...
			$reserv_info .= self::reservation_item(__('Price','booking-calendar'),(((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data["currency"] : '') . $data["price"] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data["currency"] : '')));
			$reserv_info .= self::reservation_item(__('Total','booking-calendar'),$sale_percent_html . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data["currency"] : '') . $data["total_price"] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data["currency"] : '')));
			if(count($extras_data)) {
				foreach($extras_data as $extra_data) {
					$reserv_info .= self::reservation_item($extra_data["group_label"],"");
					if($extra_data["price_type"] == "percent") {
						$extra_value = "<span class='price-percent'>".$extra_data["operation"].$extra_data["price_percent"]."% </span>";
						$extra_value .= "<span class='price'>".$extra_data["operation"] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data['currency'] : '') . (isset($extra_data["price"]) ? $extra_data["price"] : "") . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data['currency'] : ''))."</span></span></span>";
					} else {
						$extra_value = "<span class='price'>".$extra_data["operation"] .(((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data['currency'] : '') . $extra_data["price"] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data['currency'] : ''))."</span></span></span>";
					}	
					$reserv_info .= self::reservation_item($extra_data["label"],$extra_value); 
				}
				$reserv_info .= self::reservation_item(__("Price change",'booking-calendar'),("<span class='form_info'><span class='form_label'></span><span class='form_value'>".(($data['extras_price']<0)? "" : "+").(((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data['currency'] : '') . $data['extras_price'] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data['currency'] : ''))."</span>"));
			}
			if(count($form_data)) {
				foreach($form_data as $form_fild_data) {
					if($form_fild_data['type'] == 'countries' && trim($form_fild_data['value']) != "") {
						$reserv_info .= self::reservation_item($form_fild_data["label"],$countries[$form_fild_data["value"]]);					
					}elseif($form_fild_data['type'] == 'checkbox') {
						if($form_fild_data['value'] == "on") {
							$reserv_info .= self::reservation_item($form_fild_data["label"],"<i title='Close' class='fa fa-check'></i>");
						} else {
							$reserv_info .= self::reservation_item($form_fild_data["label"],"");
						}
					}else {
						$reserv_info .= self::reservation_item($form_fild_data["label"],$form_fild_data["value"]);					
					}
				}
			}
		}
