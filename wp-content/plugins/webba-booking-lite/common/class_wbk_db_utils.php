...
class WBK_Db_Utils {
	...
	static function getQuantityFromConnectedServices2( $service_id, $time ){
		...
		if( $autolock_mode == 'all' ){
	 	...
	 	foreach ( $arrIds as $service_id_this) {
		    ...
		    $service_schedule->buildSchedule( $midnight, false, true );
		    $total_quantity += $service_schedule->getAvailableCountSingle( $time );
		    $total_quantity += $service_schedule->getAvailableCountSingleRange( $time, $end );
		    */
	...
	public static function message_placeholder_processing_multi_service( $message, $appointment, $total_amount = null, $current_category = 0, $multi_token = null ){	
		$service = self::initServiceById( self::getServiceIdByAppointmentId( $appointment->getId() ) );
		return self::message_placeholder_processing( $message, $appointment, $service, $total_amount, $current_category, $multi_token );
	}	
	public static function message_placeholder_processing( $message, $appointment, $service, $total_amount = null, $current_category = 0, $multi_token = null ){	
		global $wbk_wording;
		...
		foreach( $extra_data_ids as $extra_id ){
			...
			$message = str_replace( $mask, $custom_field_value, $message );		   		        	        	
		}
		// end extra data			 
	...
	public static function message_placeholder_processing_gg( $message, $appointment, $service ){
		...
		// beging extra data
		$extra_data = $appointment->getExtra();
		$extra_data_html = str_replace( '###', ', ', $extra_data);
		$extra_data_ids = explode( '###', $appointment->getExtraWithFieldIds() );
		foreach( $extra_data_ids as $extra_id ){
			if( trim( $extra_id ) == '' ){
				continue;
